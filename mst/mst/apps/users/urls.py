...
urlpatterns = [
    ...
    url(r'^addresses/(?P<address_id>\d+)/$', views.UpdateDestoryAddressView.as_view()),    # 更新和删除地址
    url(r'^addresses/(?P<address_id>\d+)/default/$', views.DefaultAddressView.as_view()),   # 设置默认地址
    url(r'^addresses/(?P<address_id>\d+)/title/$', views.UpdateTitleAddressView.as_view()),    # 更新地址标题
